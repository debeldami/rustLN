...
fn total_cost(items: Vec<Box<dyn Material>>) -> f64 {
    items.iter().map(|item| item.cost()).sum()
}

fn main() {
    let carpet = Box::new(Carpet(3.0));
    let tile = Box::new(Tile(2.0));
    let wood = Box::new(Wood(1.0));

    let items: Vec<Box<dyn Material>> = vec![carpet, tile, wood];

    let total_cost = total_cost(items);
