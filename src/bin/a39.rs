...
fn spawn_light_thread(receiver: Receiver<LightMsg>) -> JoinHandle<LightStatus> {
    ...
    let result = thread::spawn(move || match receiver.recv() {
        Ok(lightMsg) => match lightMsg {
            LightMsg::ChangeColor(_, _, _) => LightStatus::On,

            LightMsg::Disconnect => LightStatus::Off,
        },
        Err(_) => LightStatus::Off,
    });
